...
(function () {
  ...
  async function loadExecutiveDashboard() {
    ...
    try {
      ...
      const priority = $("execTodayPriority");
      if (priority) {
        if (newRequests > 0) priority.textContent = "مراجعة الطلبات الجديدة أولاً";
        else if (openChats > 0) priority.textContent = "متابعة محادثات الدعم";
        else if (newStoreOrders > 0) priority.textContent = "معالجة طلبات المتجر";
        else priority.textContent = "تدقيق عام — لا إجراء عاجل";
      }
